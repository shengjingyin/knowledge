...
如果属性名 foo 出现在 myObj 中也出现在[[prototype]]链上层，那么就会发生屏蔽。myObj 中包含的 foo 属性会屏蔽原型链上层的所有 foo 属性，因为 myObj.foo 总是会选择原型链中最底层的 foo 属性。

如果 myObj 不存在 foo 属性，那么运行`myObj.foo = '新值'` 时，则会出现三种情况 ：

- 第一种情况，如果[[prototype]]链上**存在**名为 foo 的属性，且**没有设置为只读**（writable: false）那么会在 myObj 上创建屏蔽属性 foo
...
- 第二种情况，如果[[prototype]]链上**存在**名为 foo 的属性，且**设置为只读**（writable: true）；那么无法修改 foo 属性 或者在 myObj 上创建 foo 属性（如果在严格模式，还会报错）
  ...
  let myObj = Object.create(obj);
  myObj.foo = 4; // 无效；严格模式报错：Cannot assign to read only property 'foo' of object '#<Object>'
  ```

- 第三种情况，如果[[prototype]]链上存在名为 foo 的 set，那就一定会调用这个 set。foo 不会添加到 myObj。

  ```js
  let obj = {
    foo: 2,
    set foo(val) {
      console.log(val); // this -> myObj
    },
  };
  let myObj = Object.create(obj);
  myObj.foo = 4;
...
第二、第三种 情况就没有产生屏蔽效果，如果在第二第三种情况下还需要添加 foo 属性的话，需要调用`Object.defineProperty`设置
...
myObj.foo++; // myObj.foo = myObj.foo + 1, 创建屏蔽属性foo
```

因为`myObj.foo++ ` 等效于 `myObj.foo = myObj.foo + 1`，这里就是符合第一种情况，创建了屏蔽属性

### 实现 Object.create

```js
function myObjectCreate(target) {
  const obj = {};
